...
export interface Category {
  id: string
  name: string
  category_type: "income" | "expense" | "needs" | "wants" | "savings" | "transfer" | "both"
  is_system: boolean
  is_active?: boolean
  icon: string | null
  color: string | null
...
function transformBackendBudget(backendBudget: any): Budget {
  const transformed = {
    ...backendBudget,
    type: backendBudget.budget_type === "fifty_thirty_twenty" ? "50/30/20" : "custom",
...
function normalizeBudgetReport(report: any): BudgetReport {
  ...
  return {
    ...
    breakdowns: report.breakdowns?.map((b: any) => ({
      ...b,
...
function normalizeScenario(scenario: any): Scenario {
  ...
  return {
    ...
    scenario_savings_percentage: parseNumber(scenario.scenario_savings_percentage),
...
export const api = {
  ...
  // Transaction APIs
  async getTransactions(params?: TransactionParams): Promise<TransactionListResponse> {
  ...
  async getTransaction(id: string): Promise<Transaction> {
    const transaction = await fetchWithAuth<Transaction>(`/api/transactions/${id}`)
  ...
  async bulkUpdateTransactions(data: BulkUpdateTransactionsData): Promise<{ message: string; updated_count: number }> {
    return fetchWithAuth<{ message: string; updated_count: number }>("/api/transactions/bulk-update", {
      method: "POST",
      body: JSON.stringify(data),
    })
  },
  ...
  async processUpload(uploadId: string, file: File): Promise<{ message: string; summary: Upload }> {
  ...
  async getCategories(): Promise<Category[]> {
    const response = await fetchWithAuth<{ categories: Category[] }>("/api/categories")
  ...
  async createCategoryMapping(data: CreateCategoryMappingData): Promise<CategoryMapping> {
  ...
  async updateCategoryMapping(id: string, data: UpdateCategoryMappingData): Promise<CategoryMapping> {
  ...
  async testCategoryMapping(data: TestCategoryMappingData): Promise<{ category_id: string; category_name: string; matched_keyword: string }> {
    return fetchWithAuth<{ category_id: string; category_name: string; matched_keyword: string }>("/api/category-mappings/test", {
      method: "POST",
      body: JSON.stringify(data),
    })
  },
  ...
  async updateBudget(id: string, data: UpdateBudgetData): Promise<Budget> {
    ...
    if (data.type !== undefined) backendData.budget_type = data.type === "50/30/20" ? "fifty_thirty_twenty" : "custom"
    if (data.period !== undefined) backendData.period = data.period
    if (data.total_amount !== undefined) backendData.total_budget = data.total_amount
    if (data.needs_percentage !== undefined) backendData.needs_percentage = data.needs_percentage
    if (data.wants_percentage !== undefined) backendData.wants_percentage = data.wants_percentage
    if (data.savings_percentage !== undefined) backendData.savings_percentage = data.savings_percentage
  ...
  async setDefaultBudget(id: string): Promise<{ message: string }> {
    return fetchWithAuth<{ message: string }>(`/api/budgets/${id}/set-default`, {
      method: "POST",
    })
  },
  ...
  async getBudgetReports(budgetId: string): Promise<BudgetReport[]> {
    const response = await fetchWithAuth<{ reports: BudgetReport[]; total: number }>(`/api/budgets/${budgetId}/reports`)
    return response.reports.map(normalizeBudgetReport)
  },

  async generateBudgetReport(budgetId: string, data?: GenerateBudgetReportData): Promise<BudgetReport> {
    const report = await fetchWithAuth<BudgetReport>(`/api/budgets/${budgetId}/reports`, {
      method: "POST",
      body: JSON.stringify(data || {}),
    })
  ...
  async getBudgetReport(budgetId: string, reportId: string): Promise<BudgetReport> {
    const report = await fetchWithAuth<BudgetReport>(`/api/budgets/${budgetId}/reports/${reportId}`)
    return normalizeBudgetReport(report)
  },

  async getCurrentReport(budgetId: string): Promise<BudgetReport | null> {
    try {
      const report = await fetchWithAuth<BudgetReport>(`/api/budgets/${budgetId}/reports/current`)
      return normalizeBudgetReport(report)
    } catch {
      return null
    }
  ...
  async getLatestReport(budgetId: string): Promise<BudgetReport | null> {
    try {
      const report = await fetchWithAuth<BudgetReport>(`/api/budgets/${budgetId}/reports/latest`)
      return normalizeBudgetReport(report)
    } catch {
      return null
    }
  ...
  async createScenario(budgetId: string, data: CreateScenarioData): Promise<Scenario> {
    // Transform frontend data to backend schema
    const backendData = {
      name: data.name,
    ...
    const scenario = await fetchWithAuth<Scenario>(`/api/scenarios?budget_id=${budgetId}`, {
      method: "POST",
      body: JSON.stringify(backendData),
    })
    return normalizeScenario(scenario)
  ...
  async calculateScenario(id: string, data: CalculateScenarioData): Promise<ScenarioCalculation> {
    ...
    return fetchWithAuth<ScenarioCalculation>(`/api/scenarios/${id}/calculate`, {
      method: "POST",
      body: JSON.stringify(backendData),
    })
  },

  async applyScenario(id: string): Promise<{ message: string; new_budget_id: string }> {
    return fetchWithAuth<{ message: string; new_budget_id: string }>(`/api/scenarios/${id}/apply`, {
      method: "POST",
    })
  },
  ...
  async triggerAlertCheck(): Promise<{ message: string; alerts_triggered: number }> {
    return fetchWithAuth<{ message: string; alerts_triggered: number }>("/api/budgets/alerts/check", {
      method: "POST",
    })
  },

  async dismissAlert(id: string): Promise<{ message: string }> {
    return fetchWithAuth<{ message: string }>(`/api/budgets/alerts/${id}/dismiss`, {
      method: "PATCH",
    })
  },

  async markAlertRead(id: string): Promise<{ message: string }> {
    return fetchWithAuth<{ message: string }>(`/api/budgets/alerts/${id}/read`, {
      method: "POST",
    })
  },
  ...
  // Goal APIs
  async getGoals(statusFilter?: string): Promise<{ goals: Goal[]; total: number }> {
    const query = statusFilter ? `?status_filter=${statusFilter}` : ""
    return fetchWithAuth<{ goals: Goal[]; total: number }>(`/api/goals${query}`)
  },
  ...
  async recordContribution(goalId: string, amount: number, date: string): Promise<Goal> {
  ...
  async createGoalMilestone(goalId: string, data: GoalMilestoneCreate): Promise<GoalMilestone> {
    return fetchWithAuth<GoalMilestone>(`/api/goals/${goalId}/milestones`, {
      method: "POST",
      body: JSON.stringify(data),
    })
  ...
  // Recommendation APIs
  async getRecommendations(params?: { category?: string; status?: string; limit?: number; offset?: number }): Promise<{ recommendations: Recommendation[]; total: number }> {
    const queryParams = new URLSearchParams()
    ...
    return fetchWithAuth<{ recommendations: Recommendation[]; total: number }>(`/api/recommendations${query}`)
  },

  async getRecommendation(recommendationId: string): Promise<Recommendation> {
    return fetchWithAuth<Recommendation>(`/api/recommendations/${recommendationId}`)
  },

  async generateRecommendations(data?: RecommendationGenerateRequest): Promise<{ recommendations: Recommendation[]; total: number }> {
    return fetchWithAuth<{ recommendations: Recommendation[]; total: number }>("/api/recommendations/generate", {
      method: "POST",
      body: JSON.stringify(data || {}),
    })
  },

  async dismissRecommendation(recommendationId: string, data?: RecommendationDismissRequest): Promise<Recommendation> {
    return fetchWithAuth<Recommendation>(`/api/recommendations/${recommendationId}/dismiss`, {
      method: "PATCH",
      body: JSON.stringify(data || {}),
    })
  },

  async implementRecommendation(recommendationId: string): Promise<Recommendation> {
    return fetchWithAuth<Recommendation>(`/api/recommendations/${recommendationId}/implement`, {
      method: "PATCH",
    })
  },

  async snoozeRecommendation(recommendationId: string, data?: RecommendationSnoozeRequest): Promise<Recommendation> {
    return fetchWithAuth<Recommendation>(`/api/recommendations/${recommendationId}/snooze`, {
      method: "PATCH",
