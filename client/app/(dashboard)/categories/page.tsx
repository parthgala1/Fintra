...
export default function CategoriesPage() {
  ...
  const groupedCategories = {
    ...
    savings: filteredCategories.filter(cat => cat.category_type === "savings"),
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-[#020617]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            ...
            <Link href="/categories" className="text-sm font-medium text-green-400">Categories</Link>
          </nav>
      ...
      <main className="pt-24 pb-12">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-8">
            ...
            <p className="text-slate-400">Manage your transaction categories.</p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {Object.entries(groupedCategories).map(([type, cats]) => (
                <div key={type} className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden">
                  <div className={`border-b border-white/10 ${categoryColors[type as keyof typeof categoryColors]} px-6 py-4`}>
                    <h2 className={`text-lg font-semibold capitalize ${categoryTextColors[type as keyof typeof categoryTextColors]}`}>
                      {type} ({cats.length})
                    </h2>
                  </div>
                  <div className="divide-y divide-white/5">
                    {cats.length === 0 ? (
                      ...
                      cats.map((category) => (
                        <div key={category.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/5 transition-colors">
                          {editingId === category.id ? (
                            ...
                            <>
                              <div className="flex items-center gap-3">
                                ...
                                >
                                  <Tag className="h-4 w-4" style={{ color: category.color || "#22c55e" }} />
                                </div>
                                <div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, category_type: e.target.value as "income" | "needs" | "wants" | "savings" | "transfer" })}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 uppercase mb-2">Color</label>
                  ...
                  {COLOR_OPTIONS.map((color) => (
                    <button
                      ...
                      className={`h-8 w-8 rounded-lg transition-transform ${
                        formData.color === color ? "ring-2 ring-white ring-offset-2 ring-offset-[#0f172a] scale-110" : ""
                      }`}
