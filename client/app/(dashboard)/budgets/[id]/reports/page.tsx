...
export default function BudgetReportsPage({ params }: BudgetReportsPageProps) {
  ...
  const [currentReport, setCurrentReport] = useState<BudgetReportType | null>(null)
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      await api.generateBudgetReport(budgetId, { period_start: periodStart, period_end: periodEnd })
      // Refetch the reports
      const [reportsData, currentData, budgetData] = await Promise.all([
        api.getBudgetReports(budgetId),
        api.getCurrentReport(budgetId),
              ...
              <div className="grid gap-6 md:grid-cols-4">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-green-400 mt-1">
                    {formatAmount(currentReport.total_budgeted - currentReport.total_spent)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-slate-400">Deviation</p>
                  <p className={`text-xl font-bold mt-1 flex items-center gap-1 ${getDeviationColor(currentReport.overall_deviation ?? 0)}`}>
                  ...
                  <div key={report.id} className="p-6 hover:bg-white/5 transition-colors">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          ...
                          <p className="font-semibold text-white">{formatAmount(report.total_spent)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm text-slate-400">Budgeted</p>
                          <p className="font-semibold text-white">{formatAmount(report.total_budgeted)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm text-slate-400">Deviation</p>
                          <p className={`font-semibold flex items-center gap-1 ${getDeviationColor(report.overall_deviation ?? 0)}`}>
                    ...
                    {report.category_breakdown && report.category_breakdown.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-white/10">
                        <p className="text-xs font-medium text-slate-400 uppercase mb-3">Category Breakdown</p>
                          ...
                          {report.category_breakdown.map((cat) => (
                            <div key={cat.category_id} className="flex items-center justify-between text-sm">
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <span className={`font-medium ${getDeviationColor(cat.deviation ?? 0)}`}>
